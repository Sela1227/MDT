...
| **P0-7** | `deleteSharedSlide` 只是新增刪除 commit,**檔案永留 Git 歷史**;分享清單 `slice(0,30)` 導致第 31 筆後**UI 再也管不到** | 畫面顯示「已刪除」是謊報;管不到的公開病人檔案隨時間累積 | ①刪除時據實告知 Git 歷史;②分享清單移除 30 筆上限;③補「列出 GitHub 上所有檔案」比對功能 |
