...
# 5. zip 檔名 = 版本號
# 6. 版本號命名規則（嚴格遵守）：
#    bug fix / hotfix      → +0.01  （如 V3.2.11 → V3.2.11）
#    新功能 / 新欄位       → +0.1   （如 V3.2.11 → V3.2.11）
#    大改版 / 架構重構     → +1.0   （如 V3.2.11 → V4.0.0）
...
| 版本 | 關鍵變更 |
|------|---------|
| V3.2.11 | LINE通知移除診斷；HTML投影片診斷字放大；topics固定順序（影像/病理/分期/治療策略）|
...
| V3.0.1 | 移除isOwn編輯限制，任何登入者可編輯所有癌別 |
...
| V2.5.3 | Exam/治療textarea多行；特殊議程類別chips |
| V2.5.2 | PH chips delegation修正；欄位比例調整 |
| V2.5.1 | 修drSelect2→drSelDept；新增toggleTopic；更新genTestData |
| V2.5.0 | 個案表單大改版：PHchips、病理欄、Exam/治療條列、M/F性別 |
| V2.4.2 | 影像說明欄（上傳時可填說明）；閱覽模式顯示影像清單+說明（無縮圖）+預覽按鈕 |
| V2.4.1 | BUG-001修正：buildImgArea改用data-action+document事件委派，影像按鈕真正可用 |
| V2.4.0 | 追蹤說明/討論要點改textarea；addImg加setTimeout+addEventListener |
| V2.3.1 | PPTX 前期追蹤加說明列；DOCX 個案討論聚焦摘要/結論 |
| V2.3.0 | 手機底部 tab 導覽 + 頂部列 + 使用者切換 Sheet |
| V2.2.4 | 日曆格子四邊框（CSS border-top/left on container + border-right/bottom on cells）；renderCalendar 改 DOM 操作 |
...
| V2.1.9 | 新增 migrateDRS() + migrateCFGConv() 遷移函數，自動修正 localStorage 舊資料（腫瘤內科→消化內科/血液腫瘤科，侯柏年移核醫科，conv 補 dept 前綴） |
| V2.1.8 | conv 欄位補 dept:: 前綴（召集人下拉可正確預選）；breast/lymphoma/hepatobiliary 補 defaultSlot |
| V2.1.7 | 腫瘤內科→消化內科；張正雄/劉大智移血液腫瘤科；侯柏年移出影像診斷科；editDr 名稱去「醫師」後綴；預載資料更新正確科別 |
| V2.1.6 | 腫瘤外科→一般外科；王威群移頭頸外科；歐金俊移一般外科；聯絡人唯讀；召集人下拉修正 |
| V2.1.5 | 醫師科別合併；時間改二選一下拉；聯絡人唯讀；README 重整 |
| V2.1.4 | 癌別設定 DOM 渲染（修正空白行）；醫師科別預設收合；預載資料重設計 |
| V2.1.3 | 癌別設定各自儲存按鈕 |
| V2.1.2 | 聯絡人自動帶入；Zoom 新建空白；defaultSlot |
| V2.1.1 | 修正 deleteMtg 漏到 `</html>` 之後 |
| V2.1.0 | 刪除四碼確認；影像上傳 body.appendChild fix |
| V2.0.5 | 移除 17 重複函數、36 廢棄 CSS |
| V2.0.0 | 影像支援；PPTX 全新排版；智慧開啟模式 |
| V1.9.0 | 行事曆首頁；weeknum；歷史記錄月份分組 |
| V1.8.0 | 行事曆首頁上線 |
| V1.7.0 | 合併開會分 tab 編輯；地點/時間/召集人改下拉 |
| V1.6.0 | Noto Sans TC；合併開會分開存 |
